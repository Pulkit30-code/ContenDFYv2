...
export function DashboardSidebar({ email }: { email: string | null }) {
  ...
  const renderNav = (items: NavItem[]) => items.map(({ label, href, icon: Icon, indicator }) => {
    ...
    return <Link key={label} href={href} aria-current={active ? "page" : undefined} title={collapsed ? label : undefined} className={navClass(active)}><Icon /><span className={collapsed ? "sr-only" : ""}>{label}</span>{!collapsed && badge > 0 && <span className={`ml-auto flex min-w-5 items-center justify-center rounded-full px-1.5 py-0.5 text-[10px] font-semibold ${label === "Notifications" ? "bg-violet-500 text-white" : "bg-white/[.08] text-zinc-400"}`}>{badge}</span>}{!collapsed && indicator && <span className="ml-auto size-2 rounded-full bg-emerald-400 shadow-[0_0_10px_rgba(74,222,128,.8)]" />}{activeDot(active)}</Link>;
  });
  const utility = (label: string, href: string, Icon: typeof UserRound) => <Link href={href} aria-current={isActive(href) ? "page" : undefined} title={collapsed ? label : undefined} className={navClass(isActive(href))}><Icon /><span className={collapsed ? "sr-only" : ""}>{label}</span>{activeDot(isActive(href))}</Link>;
  ...
  useEffect(() => {
    ...
    const ensureExpandedMobileNavigation = (event: MediaQueryListEvent | MediaQueryList) => {
      if (event.matches) setCollapsed(false);
    };
    window.addEventListener("cfy:open-mobile-navigation", openMobileNavigation);
    mobileViewport.addEventListener("change", ensureExpandedMobileNavigation);
    return () => {
      window.removeEventListener("cfy:open-mobile-navigation", openMobileNavigation);
      mobileViewport.removeEventListener("change", ensureExpandedMobileNavigation);
    };
  }, []);
  useEffect(() => {
  ...
  return <div className={`h-0 shrink-0 lg:h-svh ${collapsed ? "lg:w-20" : "lg:w-64"}`}><button type="button" onClick={closeMobileNavigation} aria-label="Close navigation" className={`fixed inset-0 z-[110] bg-black/65 backdrop-blur-sm transition-opacity duration-300 lg:hidden ${mobileOpen ? "visible opacity-100" : "invisible pointer-events-none opacity-0"}`} /><aside id="workspace-navigation" aria-label="Workspace navigation" className={`cfy-sidebar fixed inset-y-0 left-0 z-[120] flex h-svh w-[min(20rem,88vw)] flex-col overflow-y-auto transition-[translate,width,padding] duration-300 lg:z-30 lg:translate-x-0 lg:overflow-visible ${mobileOpen ? "translate-x-0" : "-translate-x-full"} ${collapsed ? "lg:w-20 px-2 py-4" : "lg:w-64 px-3 py-4"}`} onClick={(event) => { if ((event.target as HTMLElement).closest("a")) closeMobileNavigation(); }}>
    <div className={`relative flex items-center ${collapsed ? "flex-col gap-3 pt-7" : "gap-2 px-2"}`}>
      ...
      <button onClick={closeMobileNavigation} className="ml-auto flex size-11 shrink-0 items-center justify-center rounded-xl border border-white/[.12] bg-[#152039] text-zinc-300 transition hover:border-violet-300/50 hover:text-white lg:hidden" aria-label="Close navigation"><X className="size-4" /></button>
      <button onClick={() => setCollapsed((value) => !value)} className={`${collapsed ? "absolute right-0 top-0" : "ml-1"} hidden size-9 shrink-0 items-center justify-center rounded-xl border border-white/[.12] bg-[#152039] text-zinc-300 transition hover:border-violet-300/50 hover:text-white lg:flex`} aria-label={collapsed ? "Expand navigation" : "Collapse navigation"}>{collapsed ? <ChevronRight className="size-4" /> : <ChevronLeft className="size-4" />}</button>
    ...
    <nav className={`${collapsed ? "mt-7" : "mt-3"} space-y-0.5 [&_.cfy-nav-item]:h-11 lg:[&_.cfy-nav-item]:h-9`}>{renderNav(visiblePrimaryNav)}</nav>
    {!collapsed && <p className="mt-5 px-3 text-[10px] font-semibold uppercase tracking-[.16em] text-zinc-600">People</p>}
    <nav className={`${collapsed ? "mt-4" : "mt-1.5"} space-y-0.5 [&_.cfy-nav-item]:h-11 lg:[&_.cfy-nav-item]:h-9`}>{renderNav(visiblePeopleNav)}</nav>
    {!collapsed && <p className="mt-5 px-3 text-[10px] font-semibold uppercase tracking-[.16em] text-zinc-600">Workspace</p>}
    <nav className={`${collapsed ? "mt-4" : "mt-1.5"} space-y-0.5 [&_.cfy-nav-item]:h-11 lg:[&_.cfy-nav-item]:h-9`}>{utility("Profile", "/workspace/profile", UserRound)}{canManageWorkspace(role) && utility("Settings", "/workspace/settings", Settings)}</nav>
